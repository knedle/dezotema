# DezoTema

Tato wiki zachycuje **názory, postoje a tvrzení** z přepisů videí a rozhovorů vybraných českých a slovenských komentátorů, politiků a aktivistů.

Obsah je organizován **tematicky** — každá stránka pokrývá jedno téma napříč všemi zdroji. Tvrzení jsou prezentována jako postoje konkrétních mluvčích, nikoli jako ověřená fakta.

## Zpracované zdroje

- [Jindřich Rajchl](https://www.facebook.com/jindrich.rajchl) — předseda strany PRO, právník
- [Lenka Tarabová](https://www.facebook.com/lenka.tarabova) — aktivistka, komentátorka
- [Konspirátor Boldy / KonspyChannel](https://www.facebook.com/KonspyChannel) — konspiračně-okultistický komentátor
- [Pavel Zítko / Československo TV2](https://www.facebook.com/CeskoslovenskoTV2) — alternativní médium
- [Jiří Černohorský Živě](https://www.facebook.com/cernohorsky.jiri) — aktivista, komentátor
- [Ladislav Vrabel](https://www.facebook.com/ladislav.vrabel.2025) — organizátor demonstrací, komentátor večerních zpráv

## Témata

| Téma | Popis |
|------|-------|
| [Benešovy dekrety a sudetský sjezd](benesovy-dekrety.md) | Benešovy dekrety, sudetoněmecký Landsmannschaft a sjezd v Brně, Posselt, válečné reparace |
| [Blízký východ](blizky-vychod.md) | Izrael a Palestina, Gaza, Írán, Hormuzský průliv |
| [Ceny energií](ceny-energie.md) | Pohonné hmoty, ruský plyn a ropa, zdražování, mechanismy cen |
| [COVID a vakcíny](covid-vakciny.md) | Tvrzení o pandemii, vakcínách, PCR testech |
| [Digitální kontrola](digitalni-kontrola.md) | CBDC a digitální euro, 6G, digitální ID, sledování (Pegasus, odposlech) |
| [DSA a cenzura](dsa-cenzura.md) | Nařízení EU o digitálních službách, svoboda projevu, blokování na sítích |
| [FARA a neziskovky](fara-neziskovky.md) | Zákon o transparentnosti zahraničního financování neziskovek |
| [Kauzy a korupce](kauzy.md) | Dozimetr, Bitcoin kauza, Hřib, Kettner, F-35 |
| [Konspirační teorie](konspiraci.md) | Konspirační teorie, elity, okultismus, QAnon |
| [Média a propaganda](media-propaganda.md) | Kritika mainstreamových médií, algoritmy, dezinformace |
| [Migrace a islamizace](migrace.md) | Migrační pakt, islamizace Západu, ukrajinská migrace |
| [Politický systém](politicky-system.md) | Demokracie, volby, politická kultura |
| [Rodina a děti](rodina-deti.md) | Rodičovská práva, gender a LGBT, demografická krize |
| [Suverenita a EU](suverenita-eu.md) | Euro, EU, Green Deal, Evropský parlament a V4 |
| [Válka, NATO a Rusko](valka-nato-rusko.md) | Válka na Ukrajině, eskalace NATO–Rusko, zbrojení, mírová jednání |
| [Větrné elektrárny](vetrniky.md) | Akcelerační zóny, technická a environmentální kritika |

### Speciální přehledy

| Přehled | Popis |
|------|-------|
| [Predikce](predikce.md) | Chronologický přehled konkrétních předpovědí — k pozdějšímu vyhodnocení |
| [Stochastický teror](stochasticky-teror.md) | Sledované výroky cílící na konkrétní osoby či skupiny |

---

Úplný seznam zpracovaných přepisů: [Zdroje](_zdroje.md)
